...
from keras.layers import Dense, Input, BatchNormalization, Conv2D, MaxPool2D, Flatten
...
def LayerCreate(layerNum, maxNode, Activation, model, filters, Kernals, room=None):
    nodeList = []
    if room == None:
        room = 2
    #CNN
    model.add(Conv2D(filters, kernel_size=(Kernals, 1), strides=(3,1), activation='relu', input_shape = (input_size,1,1)))
    model.add(MaxPool2D(pool_size=(10, 1)))
    model.add(Flatten())
    print(Kernals, filters)
    
    node = maxNode
    # encoder
    model.add(Dense(maxNode, activation=Activation))
...
space = {
    ...
    'filters' : hp.randint('filters', 32),
    'Kernals' : hp.randint('Kernals', 128),
	'activation' : hp.choice('activation',['relu','tanh','sigmoid']),
	'units' : hp.randint('units', 512),
	'layers_count' : hp.choice('layers_count', [0,1,2,3,4,5]),
	'loss_func' : hp.choice('loss_func', ['mae','mse']),
}
...
def AutoEncoder(params):    
    ...
    for j in range(output_size):
        ...
        LayerCreate(Layer, Node, Activation, model, filters, Kernals, 2)
        print(model.summary())
        adam = Adam(lr=LR)
        model.compile(optimizer='adam', loss=Loss_func)
        ES_Acc = EarlyStopping(monitor='val_loss',min_delta=0, mode='min', verbose=1, patience=50)
        history = model.fit(np.reshape(x_train[j*ClassSampleNum:((j+1)*ClassSampleNum)-1,:],(149,8192,1,1)), x_train[j*ClassSampleNum:((j+1)*ClassSampleNum)-1,:], 
        epochs=300, batch_size=int(batch_size), shuffle=True, callbacks=([ES_Acc]), 
        validation_data=(np.reshape(x_varif[(j*TestSetNum):(j+1)*TestSetNum-1],(74,8192,1,1)), x_varif[(j*TestSetNum):(j+1)*TestSetNum-1])) 
...
# 分配驗證與測試資料  (*評估輸入shape問題，重新分配)
    ...
    for i in range(output_size):
        ...
        for j in range(output_size*TestSetNum):
            cc.append(class_model[i].evaluate(np.reshape(Eval_Test_Data[j],(1,8192,1,1)),Eval_Test_Data[j]))
        Test_Eval_loss.append(cc)
        cc = []
        for j in range(output_size*verifSetNum):
            cc.append(class_model[i].evaluate(np.reshape(Eval_Varif_Data[j],(1,8192,1,1)),Eval_Varif_Data[j]))
    ...
    plt.xticks(np.arange(7)*75, ['1', '75  1    ', '75  1    ', '75  1    ', '75       '])
    ...
    with open('GS_AE_0918_5p_layer.csv', 'w', newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(['Set', 'loss', 'Layer','Node','LR','Activation','loss_Func', 'ACC'])
        for i in range(len(loss_set)):
            writer.writerow([i+1,  loss_set[i], layer_set[i], node_set[i], lr_set[i], activation_set[i], loss_func_set[i], ACC_set[i]])
